...
export default function HelpPanel() {
  ...
  const faqs = [
    {
      question: '如何上传图片或视频？',
      answer: '点击"媒体库"标签，然后点击"上传图片/视频"按钮。选择文件后，文件会自动上传并显示在媒体库中。你也可以直接把文件拖拽到上传区域。',
      icon: '📸'
    },
    {
      question: '如何创建一篇文章？',
      answer: '点击"内容管理"标签，然后点击"+ 创建新内容"按钮。按照三个步骤填写信息：1. 选择文章类型和标题 2. 填写文章内容 3. 预览并发布。每个步骤都有清晰的说明。',
      icon: '✍️'
    },
    {
      question: '草稿和发布有什么区别？',
      answer: '草稿是保存但不发布的文章，只有你能在管理后台看到。发布后，文章就会出现在网站上，所有访客都能看到。建议先保存为草稿，检查没问题后再发布。',
      icon: '📝'
    },
    {
      question: '文章可以编辑吗？',
      answer: '当然可以！在"内容管理"页面找到你的文章，点击"编辑"按钮。修改完成后，可以保存为草稿或重新发布。已发布的文章编辑后不会自动重新上线，需要点击"发布"按钮。',
      icon: '✏️'
    },
    {
      question: '支持哪些图片和视频格式？',
      answer: '图片支持 JPG、PNG、GIF、WebP 格式。视频支持 MP4、WebM、MOV 格式。单个文件最大 10MB。',
      icon: '🎬'
    },
    {
      question: '如何删除不需要的内容？',
      answer: '在"内容管理"页面找到要删除的内容，点击"删除"按钮。系统会提示确认，确认后内容将被永久删除，无法恢复，请谨慎操作。',
      icon: '🗑️'
    },
    {
      question: '文章会自动保存吗？',
      answer: '不会自动保存。请记得点击"保存草稿"按钮来保存你的工作。建议每写一段就保存一次，避免意外丢失。',
      icon: '💾'
    },
    {
      question: '如何预览文章？',
      answer: '在创建或编辑文章的第三步，你可以看到文章的预览卡片，展示封面图、标题和引言。确认无误后再发布。',
      icon: '👀'
    }
  ]
            ...
            <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-2xl">
              ...
              <div className="p-8 space-y-8">
                    ...
                    {faqs.map((faq, index) => (
                      ...
                      >
                        <summary className="px-6 py-4 cursor-pointer hover:bg-gray-100 transition-colors flex items-center gap-3">
                          ...
                          <svg className="w-5 h-5 ml-auto text-gray-500 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <section className="bg-blue-50 rounded-2xl p-6 text-center">
                  ...
                  <h3 className="text-lg font-bold text-gray-900 mb-2">还有问题？</h3>
