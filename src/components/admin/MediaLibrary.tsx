  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-xs text-gray-400 mt-1">支持 JPG, PNG, GIF, WebP, MP4, WebM（最大10MB）</p>
      </div>
        ...
        {filteredMedia.map(item => (
          <div
            ...
            className={`relative group cursor-pointer border-2 rounded-lg overflow-hidden transition-all ${
              selectedMedia.includes(item.id) ? 'border-primary' : 'border-transparent hover:border-gray-300'
            }`}
          >
            {item.type === 'image' ? (
              <img
