...
export class SEORuleEngine {
  ...
  constructor() {
    this.rules = {
      [PageType.RECOMMENDATION]: {
        ...
        descriptionTemplate: 'Discover the best {keyword} for {intent}. Our experts review and rank top solutions. Compare features, pricing, and find your perfect match.',
        ...
        requiredSections: ['comparison-table', 'detailed-reviews', 'selection-guide', 'faq'],
        suggestedLength: {
          title: { min: 50, max: 60 },
      ...
      [PageType.REVIEW]: {
        ...
        descriptionTemplate: 'Comprehensive {productName} review. We tested features, performance, and pricing. Read our honest analysis before you buy.',
        ...
        requiredSections: ['background', 'features', 'performance', 'pros-cons', 'conclusion', 'faq'],
        suggestedLength: {
          title: { min: 50, max: 60 },
          description: { min: 150, max: 160 },
      ...
      [PageType.COMPARISON]: {
        ...
        descriptionTemplate: 'Compare {productA} vs {productB} side by side. We analyze features, pricing, and performance to help you choose the best option.',
        ...
        requiredSections: ['comparison-table', 'scenario-comparison', 'final-recommendation', 'faq'],
        suggestedLength: {
          title: { min: 50, max: 60 },
          description: { min: 150, max: 160 },
          content: { min: 1500, max: 3000 }
      ...
      [PageType.TUTORIAL]: {
        ...
        descriptionTemplate: 'Learn how to {action} with our comprehensive step-by-step guide. Includes screenshots, best practices, and troubleshooting.',
        ...
        requiredSections: ['background', 'prerequisites', 'steps', 'troubleshooting', 'faq'],
        suggestedLength: {
          title: { min: 50, max: 60 },
          description: { min: 150, max: 160 },
      ...
      [PageType.RESOURCE]: {
        ...
        descriptionTemplate: 'Get our comprehensive {title} guide. Includes {valueProposition} for {targetAudience}.',
        ...
        requiredSections: ['value-proposition', 'target-audience', 'contents', 'pricing', 'faq'],
        suggestedLength: {
          title: { min: 50, max: 60 },
          description: { min: 150, max: 160 },
  ...
  // 生成Schema.org数据
  generateSchema(pageType: PageType, variables: Record<string, any>, url: string): SchemaData {
    ...
    switch (pageType) {
      case PageType.RECOMMENDATION:
        return {
          ...baseSchema,
          '@type': 'ItemList',
          itemListElement: (variables.solutions || []).map((item: Solution, index: number) => ({
            '@type': 'ListItem',
            position: index + 1,
            item: {
              '@type': 'Product',
  ...
  // 生成面包屑Schema
  generateBreadcrumbSchema(breadcrumbs: Array<{ name: string; url: string }>): SchemaData {
    return {
      '@context': 'https://schema.org',
      '@type': 'BreadcrumbList',
  ...
  // 验证SEO配置
  validateSEO(pageType: PageType, seo: SEOConfig): { valid: boolean; errors: string[] } {
    const rule = this.rules[pageType]
    const errors: string[] = []

    if (seo.title.length < rule.suggestedLength.title.min) {
      errors.push(`Title is too short (minimum ${rule.suggestedLength.title.min} characters)`)
    }
    if (seo.title.length > rule.suggestedLength.title.max) {
      errors.push(`Title is too long (maximum ${rule.suggestedLength.title.max} characters)`)
    }
    if (seo.description.length < rule.suggestedLength.description.min) {
      errors.push(`Description is too short (minimum ${rule.suggestedLength.description.min} characters)`)
    }
    if (seo.description.length > rule.suggestedLength.description.max) {
      errors.push(`Description is too long (maximum ${rule.suggestedLength.description.max} characters)`)
    }
  ...
  // 生成H标签建议
  generateHeadingSuggestions(pageType: PageType, variables: Record<string, any>): string[] {
    const rule = this.rules[pageType]
    return rule.h2Templates.map(template => this.replaceVariables(template, variables))
  }
