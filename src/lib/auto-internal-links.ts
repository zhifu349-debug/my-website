...
export class AutoInternalLinkSystem {
  ...
  insertInternalLinks(
    ...
    links.forEach(link => {
      ...
      if (insertionRegex.test(modifiedContent) && insertedLinks.length < maxInsertions) {
        modifiedContent = modifiedContent.replace(insertionRegex, (match, p1, offset) => {
          // 避免重复替换同一位置
  ...
  getLinkStatistics(): {
    ...
    return {
      ...
      averageLinksPerPage: this.pages.length > 0 ? totalLinks / this.pages.length : 0,
