...
export class AnalyticsTracker {
  ...
  // 更新指标
  private updateMetrics(pageId: string, updates: {
    views?: number
    clicks?: number
    impressions?: number
    revenue?: number
    ...
    // 重新计算派生指标
    pageMetric.rpm = pageMetric.views > 0 ? (pageMetric.revenue / pageMetric.views) * 1000 : 0
    pageMetric.conversionRate = pageMetric.clicks > 0 ? (pageMetric.views / pageMetric.clicks) * 100 : 0
  }
  ...
  // 获取聚合指标
  getAggregatedMetrics(pageId: string, days: number = 30): {
    totalViews: number
    totalClicks: number
  ...
  // 获取表现最佳页面
  getTopPerformingPages(limit: number = 10, sortBy: 'views' | 'revenue' | 'rpm' = 'revenue'): Array<{
    pageId: string
    totalViews: number
    totalRevenue: number
    averageRPM: number
    ...
    const sortKeyMap: Record<'views' | 'revenue' | 'rpm', keyof typeof results[0]> = {
      views: 'totalViews',
