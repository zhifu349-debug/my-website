...
import { CMSContent, MediaFile, Category, Tag, ContentStatus } from './cms-types'
...
class ContentStore {
    ...
    const categories: Category[] = [
      {
        ...
        slug: 'vps',
        description: { en: 'VPS hosting reviews and comparisons', zh: 'VPS主机评测和对比' },
        order: 1,
        icon: '🚀',
      ...
      },
      {
        ...
        slug: 'tutorials',
        description: { en: 'Technical tutorials and guides', zh: '技术教程和指南' },
        order: 3,
        icon: '📚',
  ...
  async createContent(content: Omit<CMSContent, 'id' | 'createdAt' | 'updatedAt'>): Promise<CMSContent> {
  ...
  async publishContent(id: string): Promise<CMSContent | null> {
    return this.updateContent(id, { status: 'published', publishedAt: new Date() })
  }
  ...
  async createMedia(media: Omit<MediaFile, 'id' | 'uploadedAt'>): Promise<MediaFile> {
